;(function ($) {
	...
	var StarFactory = {
		...
		_getWeightedRandomSize: function (sizes) {
			var weight = [0.8, 0.15, 0.05];
			return this._getWeightedRandom(sizes, weight);
		},
	...
	$.fn.starfield = function (options) {
		...
		var canvas = $('<canvas id="rocketwagon-canvas">')
			.css({position: 'absolute', left: 0, top: 0, width: '100%', height: '100%',display:'none'})
			.attr({width: $this.width(), height: $this.height()})
			.prependTo($this);

		for (var i = 0; i < numStars; i++) {
		    Starfield.push(StarFactory.getRandomStar(settings.sizes));
		}

		// ANIMATION HANDLER
		var recalcMovement = function () {
			$.each(Starfield, function (key, star) {
			    var newX = star.x - settings.deltaX;
			    var newY = star.y - settings.deltaY;
		...
		var draw = function () {
			...
			var dataurl = document.getElementById('rocketwagon-canvas').toDataURL();
